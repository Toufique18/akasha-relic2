 ...
 * @param onProgress Optional callback for tracking upload progress
 * @param abortSignal Optional AbortSignal for cancelling the upload
 * @param uploadMutation Optional RTK Query mutation for uploading
 * @returns Promise resolving to the URL of the uploaded image
 */
export const handleImageUpload = async (
  file: File,
  onProgress?: (event: { progress: number }) => void,
  abortSignal?: AbortSignal,
  uploadMutation?: (file: File) => Promise<any>
): Promise<string> => {
  // Validate file
  if (!file) {
    throw new Error("No file provided");
  ...
  try {
    // If mutation is provided, use it for real upload
    if (uploadMutation) {
      const result = await uploadMutation(file);
...
export const handleImageDelete = async (
  imageUrl: string,
  deleteMutation?: (url: string) => Promise<any>
): Promise<void> => {
  if (!imageUrl) {
  ...
  try {
    if (deleteMutation) {
      const result = await deleteMutation(imageUrl);
    ...
    throw new Error("No delete mutation provided");
  } catch (error) {
    throw new Error(
...
/**
 * Handles bulk image deletion
 * @param imageUrls Array of image URLs to delete
 * @param bulkDeleteMutation Optional RTK Query mutation for bulk deleting
 * @returns Promise resolving when all deletions are complete
 */
export const handleBulkImageDelete = async (
  imageUrls: string[],
  bulkDeleteMutation?: (urls: string[]) => Promise<any>
): Promise<void> => {
  ...
  try {
    if (bulkDeleteMutation) {
      const result = await bulkDeleteMutation(imageUrls);

      if (result?.data?.success) {
        return;
      }

      throw new Error(result?.data?.message || "Failed to delete images");
    }

    throw new Error("No bulk delete mutation provided");
  } catch (error) {
    throw new Error(
